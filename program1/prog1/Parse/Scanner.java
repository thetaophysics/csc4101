...
public class Scanner {
	...
	public Token getNextToken() {
		...
		try {
			...
			while((ch == ' ') || (ch == '\n') || (ch == '\t')) {
				ch = in.read();
				while((ch == -1) || (ch == '\n')) {	//EOF or new line character
					ch = in.read();
				}
			//2 nested loops: 
			//1st loop: comment, whrite space
			//2nd inner loop: beginning of comment, throwing away until you find a new line char or you reach EOF
			}
			...
			// String constants
				...
				return new StrToken(buf);	//create a string object,[GG] take a byte array as elements
